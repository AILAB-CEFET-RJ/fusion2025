...
2- Download the datasets from [zenodo](https://zenodo.org/uploads/14941806). For example, we'll download ERA5+SIA dataset:
```sh
wget https://zenodo.org/records/14941806/files/ERA5+SIA.zip?download=1
unzip ERA5+SIA.zip
```
...
# Example:
# fusion2025-spatiotemporal-precipitation-nowcasting
# └── STConvS2s
#     └── data
#         └── ERA5+SIA.nc
```
...
5- Download the trained model from zenodo and place it in the `trained_models` folder. For example, download ERA5+SIA model:

```sh
wget https://zenodo.org/records/14941806/files/ERA5+SIA-model.pth.tar?download=1
mkdir trained_models
mv ERA5+SIA-model.pth.tar ./trained_models/ERA5+SIA-model.pth.tar
```

Optionally, instead of downloading the already trained model from zenodo, we may execute the STConvS2S train script with the instructions below to train the model from scratch.
```sh
...
6- Evaluate the model with the MAE, Bias, Confusion Matrix and F1-Score metrics.
...
| Dataset | Sirenes | INMET | AlertaRio |
|---------|---------|-------|-----------|
| ERA5 |  |  |  |
| ERA5+S | ✓ |  |  |
| ERA5+I |  | ✓ |  |
...
Expected output in stdout and the `inference_grid_figures/iteration_0/tp_figure.png` figure generated by the script:
```sh
...
<img src="./.github/total_precipitation_inference.png" alt="Total Precipitation - Inference" height="400px" />
